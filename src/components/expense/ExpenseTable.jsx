...
export default function ExpenseTable() {
  ...
  }, [expenses, category, startDate, endDate, debouncedSearch]);

  const handleFormSubmit = (data) => {
    if (formModal.payload) {
      updateExpense(formModal.payload.id, data);
    } else {
      addExpense(data);
    }
  };
  ...
  const handleConfirmDelete = () => {
    if (confirmModal.payload) {
      deleteExpense(confirmModal.payload.id);
    }
  };

  const columns = [
    { key: 'date', label: 'Date', render: (row) => formatDate(row.date) },
    { key: 'category', label: 'Category', render: (row) => <Badge color="rose">{row.category}</Badge> },
    { key: 'note', label: 'Note', render: (row) => row.note || '—' },
    {
      key: 'amount',
      label: 'Amount',
      render: (row) => <span className="text-rose-400 font-semibold">-{formatCurrency(row.amount, settings.currency)}</span>,
    },
    {
      key: 'actions',
      label: 'Actions',
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <Button onClick={() => formModal.open(null)}>+ Add Expense</Button>
      </div>

      {filtered.length === 0 ? (
        <EmptyState
          title="No expense records"
          message="Add your first expense entry to get started."
          action={<Button onClick={() => formModal.open(null)}>+ Add Expense</Button>}
        />
      ) : (
